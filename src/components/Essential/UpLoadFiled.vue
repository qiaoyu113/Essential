  ...
  <div>
    ...
    >
      ...
      <el-button
        ...
        :round="!isPC"
      >
        {{ butText }}（{{ imageUrl.length }}/1)
      </el-button>
      <div
        v-if="tip"
        slot="tip"
        class="el-upload__tip"
        style="float: right;padding-left:10px;line-height: 24px;color: #999;font-size: 13px;"
      >
        ( 支持扩展名：.zip、.pdf、.jpg、.png )
      </div>
    </el-upload>
    <slot />
  </div>
...
export default class extends Vue {
  ...
  handleAvatarSuccess(res: any) {
    if (res) {
      ...
      this.changeValue(this.imageUrl)
    }
  }
  ...
  beforeAvatarUpload(file: any) {
    ...
    if (!isJPG) {
      ...
      this.$message.error('上传图片只能是 ZIP、PDF、JPG、JPEG、PNG、GIF 格式!')
      return false
    }
    if (!isLt2M) {
