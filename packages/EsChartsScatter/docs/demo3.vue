...
<script lang="ts">
import { reactive, toRefs, onMounted, nextTick, ref, onUnmounted } from 'vue'
...
export default {
  ...
  setup() {
    ...
    const EsChart = reactive({
        Id: 'EsChartScatter3', // 注意：id是唯一的，不能重复
        eWidth: '700px',
        eHeight: '400px',
        options: {}
    });
    ...
    const calculateAverage = (data: any, dim: number) => {
      let total = 0;
    ...
    onMounted(()=>{
      nextTick(() => {
          ...
          es.setOption(currentOption, true)
          time = setInterval(() => {
